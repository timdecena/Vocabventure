...
export default function StudentSpellingChallenge() {
  ...
  // Fetch challenges & completed IDs
  useEffect(() => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setFeedback("⚠️ Already answered or error submitting.");
    } finally {
      setTimeout(() => nextChallenge(), 2000);
    }
  }, [isSubmitted, startTime, challenges, current, answer]);
  ...
  const getCharacterSprite = () => {
    switch(panicLevel) {
      case 2: return "/sprites/Boy_Panicking2.png";
      case 1: return "/sprites/Boy_Panicking1.png";
      default: return "/sprites/Boy_Stand.png";
    }
  };

  if (loading) return <p>🔄 Loading spelling challenges...</p>;
  if (error) return <p style={{ color: "red" }}>{error}</p>;
  if (!challenges.length) return <p>⚠️ No challenges available.</p>;
  ...
  return (
    <GameWrapper>
      ...
      {showDrowningAlert && <DrowningAlert>DANGER! WATER RISING FAST!</DrowningAlert>}
      ...
      <FloatingUI>
        ...
        <Typography variant="body2" sx={{ color: waterLevel > 70 ? "red" : waterLevel > 40 ? "orange" : "inherit" }}>
          Water Level: {Math.round(waterLevel)}% {waterLevel > 70 && "⚠️ HURRY!"}
        </Typography>

        {alreadyAnswered && <p style={{ color: "orange" }}>You already answered this challenge, but you can try again.</p>}
        ...
        {!isSubmitted ? (
          <div>
            <input
              ...
              autoFocus
              style={{ padding: "12px", fontSize: "18px", borderRadius: "8px", width: "100%", border: waterLevel > 70 ? "2px solid red" : "2px solid #ddd", margin: "10px 0", backgroundColor: waterLevel > 70 ? "#fff0f0" : "white" }}
            />
            <button
              onClick={handleSubmit}
              disabled={!timerStarted || !answer.trim()}
              style={{ marginTop: "10px", padding: "12px 24px", borderRadius: "8px", border: "none", background: (!timerStarted || !answer.trim()) ? "#ccc" : waterLevel > 70 ? "#f44336" : "#2196F3", color: "white", cursor: (!timerStarted || !answer.trim()) ? "not-allowed" : "pointer", fontSize: "16px", fontWeight: "bold" }}
            >
