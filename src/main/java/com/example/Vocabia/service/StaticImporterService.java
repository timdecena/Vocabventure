...
public class StaticImporterService {
        ...
        try {
            ...
            for (File categoryFolder : categories) {
                ...
                for (File levelFolder : levelFolders) {
                    String levelName = levelFolder.getName(); // e.g., "level1"

                    // Expect level.json inside
                    ...
                    // Check if puzzle already exists (category + level unique constraint)
    ...
    // Helper to extract level number from folder name "level1" => 1
